...
class DetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //check if user want view a already added data
        if(info.equals("old")){
            binding.saveButton.visibility=View.INVISIBLE;
            val selectedDataID=intent.getIntExtra("id",1);
            val cursor = db.rawQuery("SELECT * FROM arts WHERE id = ?", arrayOf(selectedDataID.toString()));
